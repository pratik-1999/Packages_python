...
class Gaussian(Distribution):
    ...
    def calculate_stdev(self, sample=True):
        ...
        sum_squared_diffs_mean = sum([math.pow((x-mean),2) for x in self.data])  
        n = len(self.data)
        
        if sample:
            return math.sqrt(sum_squared_diffs_mean/(n-1))
        return math.sqrt(sum_squared_diffs_mean/n)
    ...
    def plot_histogram(self):
        ...
        plt.hist(self.data)
        plt.gca().set(xlabel = 'data', ylabel = 'counts', title = 'Distribution of data points')
        plt.show()
    ...
    def pdf(self, x):
        ...
        exp_part = math.exp(-(x-self.mean)/(2*self.stdev))
        rest = 1/(self.stdev*math.sqrt(2*math.pi))
        return rest*exp_part
